...
router.post('/reset-email', async (req, res) => {
	...
	if (!new_email) return res.status(422).send('E-mail não informado')
	...
	try {
		await db.query('START TRANSACTION')
		await db.query('UPDATE usuario SET email = ? WHERE id_usuario = ?', [email, id_usuario])
		await db.query('COMMIT')
	}
	catch (error) {
		await db.query('ROLLBACK')
		return res.status(500).send('Erro ao atualizar e-mail')
	}

	res.send({ message: 'E-mail atualizado' })
})
