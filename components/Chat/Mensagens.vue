<template>
	<div id="mensagensContainer" ref="mensagensContainer" class="chat-messages flex-1 flex"
		...
		<div class="messages-container flex flex-column">
			<template v-for="(mensagem, index) in mensagensStore.getMensagens" :key="mensagem.id_mensagem">
				<div :ref="el => {if (el) addObserver(el, mensagem)}"
				     :class="{ 'mt-3': (!mensagemAnteriorMesmoRemetente(mensagem) || !mensagemAnteriorMesmoMinuto(mensagem)) && index !== 0}"
				     class="flex gap-3 align-items-start message py-1 px-1 border-round"
					...
					<div class="avatar-container">
						<Avatar
							...
							:image="useMensagensStore().chatParticipants.get(mensagem.usuario_remetente)?.profile_photo"
							:label="useMensagensStore().chatParticipants.get(mensagem.usuario_remetente)?.profile_photo ? null :remetentUserName(mensagem.usuario_remetente).charAt(0)"
							shape="circle"
							size="normal"/>
					</div>
						...
						<div class="flex flex-column">
							<p class="m-0" style="word-break: break-word">
								<template v-if="!mensagem.excluida">
									...
									<small
										v-if="mensagem.lida && mensagem.usuario_remetente === useUsuarioStore().getUsuarioId"
										style="color: deepskyblue"> ✓</small>
								</template>
							</p>
						</div>
					</div>
		...
		<Button v-if="buttonFetchMessagesVisible"
		        :label="mensagensStore.buscandoMensagens ? 'Buscando...' : 'Carregar mais'"
		        :loading="mensagensStore.buscandoMensagens"
		        class="w-2 m-auto"
		        size="small" text @click="mensagensStore.buscarMensagens()"/>
		<div id="elemento" v-if="mensagensStore.getMessagesCountDB === mensagensStore.getMessagesCount">
...
onMounted(async () => {
	...
	while (mensagensContainer.value.scrollHeight - 500 <= mensagensContainer.value.clientHeight && mensagensStore.getMessagesCount < mensagensStore.getMessagesCountDB) {
...
const handleScroll = async () => {
	const scrollTop = mensagensContainer.value.scrollTop
	const scrollHeight = mensagensContainer.value.scrollHeight
	const clientHeight = mensagensContainer.value.clientHeight

	if ((scrollTop * -1) + clientHeight >= scrollHeight) {
		if (mensagensStore.getMessagesCount < mensagensStore.getMessagesCountDB && !mensagensStore.buscandoMensagens) {
			await mensagensStore.buscarMensagens()
			await nextTick()
		}
	}
